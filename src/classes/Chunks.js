...
class Chunks {
    ...
    init () { // generate only first chunk. Note that new chunk is generated when getBlock with g parameter as true is called
        this.genChunk(0, 0);
    }
    ...
    prerenderChunk (c, x, y) { // prepare whole chunk image
        ...
        for (let i = 0; i < chunkSize; i++) {
            for (let j = 0; j < chunkSize; j++) {
                ...
                const sx = scaleMarker % 2 === 0 ? 1 : -1, sy = scaleMarker >> 1 === 0 ? 1 : -1;
                const block = t[i][j];
                cctx.save();
                cctx.translate(i * tileSize + tileSize2, j * tileSize + tileSize2);
                cctx.rotate(directionMarker * deg90);
                cctx.scale(sx, sy);
                cctx.drawImage(tiles.tiles[block.i] || tiles.tiles[2], -tileSize2, -tileSize2);
                cctx.restore();
            }
        }
    ...
    genChunk (x, y) { // generating chunks (side depending on chunkSize)
        ...
        for (let i = 0; i < chunkSize; i++) {
            for (let j = 0; j < chunkSize; j++) {
                c.t[i][j] = {i: Math.abs(this.getXY(x * chunkSize + i, y * chunkSize + j) % 1) > 0.0625 ? 0 : 1};
            }
        }
        c.g = true;
    ...
    getBlock (x, y, g) { // getting pixels (g = true, - generate chunk while not found)
        ...
        const rx = x % chunkSize, ry = y % chunkSize;
    ...
    setBlock (x, y, id) { // set pixels
        ...
        try {
            ...
            console.log(err); console.log(bx + ", " + by)
        }
    }
    ...
    getChunk (x, y) { // get only id of the chunk that refers to data.chunks[id], basing of x y
