...
class Chat {
  ...
  constructor(chatElement, chatInput, messageList) {
    ...
    this.history = [];
    /**
         * The cursor of {@link Chat#history} that
         * represents currently displayed message at {@link Chat#input}.
         * The next message is displayd when the variable is equal to length of the chat history
        */
    this.historyPos = 0;
    /** The single message storage
     * used to store the currently typed message while viewing chat history */
    this.hiCache = '';
  ...
  up(evt) {
    if (this.historyPos > 0) {
      ...
      if (this.history.length === this.historyPos) this.hiCache = this.input.value || '';
  ...
  receive({
    ...
    switch (type) {
      ...
      case 'bprivate':
        ...
        break;
      default: throw new Error('Unknown message.'); // TODO handle
    }
